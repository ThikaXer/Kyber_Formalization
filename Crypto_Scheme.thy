...
definition key_gen :: "nat \<Rightarrow> (('a fr, 'k) vec, 'k) vec \<Rightarrow> ('a fr, 'k) vec \<Rightarrow> 
                ('a fr, 'k) vec \<Rightarrow> ('a fr, 'k) vec" where 
"key_gen dt A s e = compress_vec dt (A *v s + e)"

definition encrypt :: "('a fr, 'k) vec \<Rightarrow> (('a fr, 'k) vec, 'k) vec \<Rightarrow> ('a fr, 'k) vec \<Rightarrow> 
                ('a fr, 'k) vec \<Rightarrow> ('a fr) \<Rightarrow>
                nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow>
                'a fr \<Rightarrow> (('a fr, 'k) vec) * ('a fr)" where
...
definition decrypt :: "('a fr, 'k) vec \<Rightarrow> ('a fr) \<Rightarrow> ('a fr, 'k) vec \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> 'a fr" where
...
fun f_int_to_poly :: "(int \<Rightarrow> int) \<Rightarrow> ('a fr) \<Rightarrow> ('a fr)" where
  "f_int_to_poly f = 
        to_fr \<circ>
        Poly \<circ>
...
(*
definition mod_plus_minus_poly :: "('a fr) \<Rightarrow> ('a fr)" where
  "mod_plus_minus_poly p = f_int_to_poly (\<lambda>x. x mod+- q) p"

definition mod_plus_minus_vec :: "('a fr, 'k) vec \<Rightarrow> ('a fr, 'k) vec" where
  "mod_plus_minus_vec p = map_vector mod_plus_minus_poly p"
*) 

definition compress_error_poly :: "nat \<Rightarrow> 'a fr \<Rightarrow> 'a fr" where
  "compress_error_poly d y = decompress_poly d (compress_poly d y) - y"

definition compress_error_vec :: "nat \<Rightarrow> ('a fr, 'k) vec \<Rightarrow> ('a fr, 'k) vec" where
...
lemma scalar_product_linear_left:
  "scalar_product (a+b) c = scalar_product a c + scalar_product b (c :: ('a fr, 'k) vec)"
...
lemma scalar_product_linear_right:
  "scalar_product a (b+c) = scalar_product a b + scalar_product a (c :: ('a fr, 'k) vec)"
unfolding scalar_product_def
by auto (metis (no_types, lifting) distrib_left sum.cong sum.distrib)

lemma scalar_product_assoc:
  "scalar_product (A *v s) (r :: ('a fr, 'k) vec ) = scalar_product s (r v* A)"
...
lemma kyber_correct:
  ...
  assumes "t = key_gen dt A s e"
          ...
          "set ((coeffs \<circ> of_fr) m) \<subseteq> {0,1}"
          "degree (of_fr m) < n"
...
proof -
  ...
  have coeffs_m': "\<forall>i. poly.coeff (of_fr m') i \<in> {0,1}" 
  ...
  have coeffs_m':"set ((coeffs \<circ> of_fr) m') \<subseteq> {0,1}" 
  proof -
    have "compress 1 a \<in> {0,1}" for a unfolding compress_def by auto
    then have "poly.coeff (of_fr (compress_poly 1 a)) i \<in> {0,1}" for a i
      using compress_poly_1 by presburger
    then have "set (coeffs (of_fr (compress_poly 1 a))) \<subseteq> {0,1}" for a 
      using coeffs_in_coeff[of "of_fr (compress_poly 1 a)" "{0,1}"] by simp
    then show ?thesis unfolding m'_def decrypt_def by simp
  qed
  have coeff_0pm1: "set ((coeffs \<circ> of_fr) (m-m')) \<subseteq> {of_int_mod_ring (-1),0,1}"
  proof -
    have "poly.coeff (of_fr m) i \<in> {0,1}" for i using assms(7) coeff_in_coeffs
      by (metis comp_def insertCI le_degree subset_iff zero_poly.rep_eq)
    moreover have "poly.coeff (of_fr m') i \<in> {0,1}" for i using coeffs_m' coeff_in_coeffs
      by (metis comp_def insertCI le_degree subset_iff zero_poly.rep_eq)
    ultimately have "poly.coeff (of_fr m - of_fr m') i \<in> {of_int_mod_ring (- 1), 0, 1}" for i
    ...
    then have "set (coeffs (of_fr m - of_fr m')) \<subseteq> {of_int_mod_ring (- 1), 0, 1}" 
      by (simp add: coeffs_in_coeff)
    then show ?thesis using assms(7) of_fr_diff[of m m'] by simp
  qed
  have "set ((coeffs \<circ> of_fr) (m-m')) \<subseteq> {0}"
  proof (rule ccontr)
    assume "\<not>set ((coeffs \<circ> of_fr) (m-m')) \<subseteq> {0}"
    then have "\<exists>i. poly.coeff (of_fr (m-m')) i \<in> {of_int_mod_ring (-1),1}" 
      using coeff_0pm1 
  ...
  qed
  then show ?thesis by (simp flip: m'_def) (metis to_fr_of_fr)
qed
...
definition get_Psi  where
  "get_Psi d = do{
    y <- pmf_of_set (UNIV :: ('a fr, 'k) vec set);
