...
"m mod+- b = (THE r. r = m mod b \<and> -(q div 2) < r \<and> r \<le> (q div 2))" 

definition mod_plus ::  "int \<Rightarrow> int \<Rightarrow> int" (infixl "mod+" 70) where 
"m mod+ b = (THE r. r = m mod b \<and> 0\<le>r \<and> r<b)" 

lemma mod_plus_range: assumes "x \<in> set [0..b-1]" shows "x mod+ b = x"
using assms by (auto simp add: mod_plus_def)
...
lemma range_compress_without_mod_plus:
assumes 
    "x\<in>{0..q-1}"
    "of_nat d < \<lceil>(log 2 q)::real\<rceil>"
shows "compress x d \<in> {0..2^d - 1}" 
proof (cases "x<floor (q-(q/2^(d+1)))")
case True
  have "x\<in>{0..floor (q-(q/2^(d+1)))-1}" using True assms(1) by auto
  then have "round (real_of_int (2 ^ d * x) / real_of_int q) \<in> {0..2^d - 1}" 
  ...
  qed 
  then show ?thesis unfolding compress_def
    using mod_plus_range set_upto by presburger
next
case False
  then have "x\<in>{floor (q-(q/2^(d+1))).. q-1}" using assms(1) by auto
  then show ?thesis sorry
qed

lemma decompress_compress_1: 
assumes 
    "x\<in>set [0..floor (q-(q/2^(d+1)))-1]"
    "of_nat d < \<lceil>(log 2 q)::real\<rceil>"
...
proof -
  ...
  have "let c_real = real_of_int (2 ^ d * x) / real_of_int q in 
    ...
    using mod_plus_range range_2d by auto
...
shows "abs ((x' - x) mod+- q) \<le> round ( of_int q / of_int (2^(d+1)) )"
using assms unfolding compress_def decompress_def 
apply (auto simp add: round_def mod_plus_def mod_plus_minus_def) 
